...
const getDirname = () => path.dirname(require.main?.filename || process.mainModule?.filename || __dirname);
function parseSOAPFromWord(text) {
    const soap = {
        Subjective: [],
        Objective: [],
        Assessment: [],
...
async function main() {
    try {
        ...
        const docxFiles = files.filter((f) => /\.docx?$/i.test(f)); // 获取所有.docx文件
